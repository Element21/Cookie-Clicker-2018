...
Game.Launch=function()
{
	...
	'<div class="listing">&bull; launched our <a href="https://www.patreon.com/dashnet" target="_blank">Patreon</a></div>'+
	...
	Game.Init=function()
	{
		...
		Game.tooltip.update=function()
		{
			...
			else
			{
				...
				{
					X=Game.mouseX-this.tt.clientWidth-24;
					Y=Game.mouseY-this.tt.clientHeight/2-8;
					X=Math.max(0,Math.min(Game.windowW-this.tt.clientWidth-16,X));
					Y=Math.max(0,Math.min(Game.windowH-this.tt.clientHeight-64,Y));
				}
		...
		Game.GrabDataResponse=function(response)
		{
			...
			*/
			...
			try{
				...
				{
					Game.grandmaNames=Game.grandmaNames.concat(r['grandma'].split('|'));
					Game.grandmaNames=Game.grandmaNames.filter(function(el){return el!='';});
				}
		...
		Game.computeLumpTimes=function()
		{
			...
			if (Game.hasAura('Dragon\'s Curve')) {Game.lumpMatureAge/=1.07;Game.lumpRipeAge/=1.07;}
			...
			else//draw it
			{
				...
				this.draw=function()
				{
					...
					*/
					...
					{
						var selected=-1;
						//todo : adjust for different buildings
						var marginW=-18;
						var marginH=-10;
						for (var i=0;i<len;i++)
						{
					...
					for (var i=0;i<len;i++)
					{
						...
						if (selected==i && this.name=='Grandma')
						{
							...
							var text=choose(Game.grandmaNames)+', age '+Beautify(Math.floor(70+Math.random()*30+years+this.level));
		...
		new Game.Object('Grandma','grandma|grandmas|baked|Grandmas are [X] year older|Grandmas are [X] years older','A nice grandma to bake more cookies.',1,1,{pic:function(i){
			...
			if (Game.Has('Metagrandmas')) mult*=2;
		...
		new Game.Object('Fractal engine','fractal engine|fractal engines|made from cookies|[X] iteration deep|[X] iterations deep','Turns cookies into even more cookies.',16,20,{base:'fractalEngine',xV:8,yV:64,w:64,rows:1,x:0,y:0},12345678987654321,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			if (Game.Has('Metagrandmas')) mult*=Game.getGrandmaSynergyUpgradeMultiplier(me.name);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
		},function(){
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock('Metagrandmas');
		});
		...
		order=255;
		new Game.Upgrade('Metagrandmas',Game.getGrandmaSynergyUpgradeDesc('Fractal engine')+'<q>A fractal grandma to make more grandmas to make more cookies.</q>',Game.Objects['Fractal engine'].basePrice*Game.Tiers[2].price,[10,9],function(){Game.Objects['Grandma'].redraw();});
		...
		order=5000;
		new Game.SynergyUpgrade('Compounded odds','<q>When probabilities start cascading, "never in a billion lifetimes" starts looking terribly like "probably before Monday comes around".</q>','Fractal engine','Chancemaker','synergy1');
		new Game.SynergyUpgrade('Mice clicking mice','<q>Absolutely blasphemous!</q>','Fractal engine','Cursor','synergy2');
		...
		Game.seasonDrops=Game.heartDrops.concat(Game.halloweenDrops).concat(Game.easterEggs).concat(Game.santaDrops);
		...
		Game.UpgradePositions={141:[176,-66],181:[-555,-93],253:[-272,-231],254:[-99,-294],255:[-193,-279],264:[48,123],265:[133,154],266:[223,166],267:[305,137],268:[382,85],269:[-640,42],270:[-607,-246],271:[-728,-120],272:[-688,-201],273:[-711,-33],274:[270,-328],275:[317,-439],276:[333,-556],277:[334,-676],278:[333,-796],279:[328,-922],280:[303,-1040],281:[194,-230],282:[-265,212],283:[-321,297],284:[-322,406],285:[-243,501],286:[-403,501],287:[-314,606],288:[-312,-374],289:[-375,-502],290:[-206,-476],291:[453,-745],292:[-375,-651],293:[-399,-794],323:[-215,32],325:[192,-1127],326:[-328,-158],327:[-192,290],328:[-3,237],329:[92,376],353:[121,-326],354:[77,-436],355:[64,-548],356:[57,-673],357:[52,-793],358:[58,-924],359:[82,-1043],360:[-188,408],362:[158,289],363:[-30,-30],364:[-232,-730],365:[-77,349],368:[-82,-532],393:[196,-714],394:[197,-964],395:[-124,-139],396:[-264,-889],397:[-69,563],408:[-204,-1036],409:[-72,-1152],410:[-70,-1328],411:[-388,137],412:[-470,253],413:[-482,389],449:[-367,-1113],450:[-334,-1214],451:[-278,-1303],495:[-402,-966],496:[200,49],505:[-545,-570],520:[-204,-70],537:[-937,-149],};
		...
		order=2200;
		new Game.TieredAchievement('Self-contained','Have <b>1</b> fractal engine.','Fractal engine',1);
		new Game.TieredAchievement('Threw you for a loop','Have <b>50</b> fractal engines.','Fractal engine',2);
		new Game.TieredAchievement('The sum of its parts','Have <b>100</b> fractal engines.','Fractal engine',3);
		new Game.TieredAchievement('Bears repeating','Have <b>150</b> fractal engines.<q>Where did these come from?</q>','Fractal engine',4);
		new Game.TieredAchievement('More of the same','Have <b>200</b> fractal engines.','Fractal engine',5);
		new Game.TieredAchievement('Last recurse','Have <b>250</b> fractal engines.','Fractal engine',6);
		new Game.TieredAchievement('Out of one, many','Have <b>300</b> fractal engines.','Fractal engine',7);
		new Game.TieredAchievement('An example of recursion','Have <b>350</b> fractal engines.','Fractal engine',8);
		new Game.TieredAchievement('For more information on this achievement, please refer to its title','Have <b>400</b> fractal engines.','Fractal engine',9);
		new Game.TieredAchievement('I\'m so meta, even this achievement','Have <b>450</b> fractal engines.','Fractal engine',10);
		new Game.TieredAchievement('Never get bored','Have <b>500</b> fractal engines.','Fractal engine',11);
		
		order=2220;
		new Game.Achievement('The needs of the many','Make <b>100,000,000,000,000,000,000,000,000</b> cookies just from fractal engines.',[20,22]);
		new Game.Achievement('Eating its own','Make <b>100,000,000,000,000,000,000,000,000,000</b> cookies just from fractal engines.',[20,23]);
		new Game.Achievement('We must go deeper','Make <b>100,000,000,000,000,000,000,000,000,000,000</b> cookies just from fractal engines.',[20,24]);
		...
		*/
		...
		Game.dragonLevels=[
			...
			{name:'Krumblor, cookie dragon',action:'Train Dragon Curve<br><small>Aura : sugar lumps grow 7% faster, 50% weirder</small>',pic:5,
		...
		Game.dragonAuras={
			...
			17:{name:'Dragon\'s Curve',pic:[20,25],desc:'<b>+7% sugar lump growth</b> and sugar lumps are <b>twice as likely</b> to be unusual.'},
		};
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
			{
				...
				if (Game.Has('Metagrandmas')) grandmas++;
				...
				if (Game.Objects['Fractal engine'].totalCookies>=base*		10000000000000) Game.Win('The needs of the many');
				...
				if (Game.Objects['Fractal engine'].totalCookies>=base*		10000000000000) Game.Win('Eating its own');
				...
				if (Game.Objects['Fractal engine'].totalCookies>=base*		10000000000000) Game.Win('We must go deeper');
